...
The approximation of $f_i$ involves a Taylor series expansion, and we are able to obtain an effective average of $f_i$ across all age intervals. A weight $cn_1$ can also be calculated using the probability of dying $q_i$, the intensity ratio $\psi_i$, and the probability of surviving from birth to the start of the ith interval $S_i$:
...
Here $cn_2$ is also a weight obtained through the approximation process. It can be calculated in R using the probability of dying $q_i$, the intensity ratio $\psi_i$, the fraction $\alpha_i$, the age interval of 5 years, and the probability of surviving from birth to the start of the ith interval $S_i$:
...
grid.arrange(arrangeGrob(p2, p3, ncol=2, nrow=1), heights=c(7,4), bottom = "Figure 2. ELRR and REID approximations for 2010 lung cancer mortality")
```
...
Approx_graph <- function(dx_cancer, cancer, x, f) {
  ...
  grid.arrange(p1, p2, p3, nrow=2, ncol = 2, top=title)
